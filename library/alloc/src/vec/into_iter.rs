...
impl<T, A: Allocator> IntoIter<T, A> {
    ...
    pub fn as_slice(&self) -> &[T] {
        unsafe { slice::from_raw_parts(self.buf.as_ptr().offset(self.alive.start as isize), self.len()) }
    }
...
impl<T, A: Allocator> Iterator for IntoIter<T, A> {
        ...
        Self: TrustedRandomAccess,
    {
        ...
        // them for `Drop`.
        unsafe {
            if mem::size_of::<T>() == 0 { mem::zeroed() } else { ptr::read(self.buf.as_ptr().offset(self.alive.start as isize).add(i)) }
        }
    }
}
