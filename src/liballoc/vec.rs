...
use core::iter::{FromIterator, FusedIterator, TrustedLen, SourceIter, InPlaceIterable,
                 TrustedRandomAccess};
...
impl<T, I> SpecFromNested<T, I> for Vec<T> where I: TrustedLen<Item=T> {
...
impl<T, I> SpecFrom<T, I> for Vec<T>
    where I: Iterator<Item=T> + InPlaceIterable + SourceIter<Source=IntoIter<T>>
{
    default fn from_iter(mut iterator: I) -> Self {
        ...
        let dst = if mem::needs_drop::<T>() {
            ...
            iterator.try_fold::<_, _, Result<_, !>>(dst, move |mut dst, item| {
                unsafe {
        ...
        if mem::needs_drop::<T>() {
            ...
            unsafe { ptr::drop_in_place(src.as_mut_slice()); }
        }
...
unsafe impl<T> TrustedRandomAccess for IntoIter<T> where T: Copy {
